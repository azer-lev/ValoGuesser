...
function getRoom(gameMode) {
    ...
    for (const _room of rooms) {
        ...
        if (gameMode == aufbau_1.GameMode.oneVsOne && _room.players.length == 1 ||
            gameMode == aufbau_1.GameMode.twoVsTwo && (_room.players.length < 4) && _room.players.length != 0) {
            room = _room;
        }
    }
    if (room == null) {
...
class SetPlayer //extends BodyChild
 {
    ...
    get isValid() {
        return this.playerToken != null && this.gameMode != null &&
            this.roomKey != null && this.privateLobby != null;
    }
}
/*(async async => {
